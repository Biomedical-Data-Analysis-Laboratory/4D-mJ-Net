...
def VNet_Milletari(params):
    ...
    if getUSE_PM(): # use the PMs as input and concatenate them
        ...
        if "multiInput" in params.keys():
            for pm in ["cbf", "cbv", "ttp", "mip", "mtt", "tmax"]:
                if pm in params["multiInput"].keys() and params["multiInput"][pm] == 1: list_input.append(layers.Input(shape=(getM(), getN(), 3, 1), sparse=False))
    ...
    stride_1 = (2, 2, params["strides"]["conv.1"]) if getTIMELAST() or getUSE_PM() else (params["strides"]["conv.1"], 2, 2)
    ...
    stride_2 = (2, 2, params["strides"]["conv.2"]) if getTIMELAST() or getUSE_PM() else (params["strides"]["conv.2"], 2, 2)
    ...
    stride_3 = (2, 2, params["strides"]["conv.3"]) if getTIMELAST() or getUSE_PM() else (params["strides"]["conv.3"], 2, 2)
    ...
    stride_4 = (2, 2, params["strides"]["conv.4"]) if getTIMELAST() or getUSE_PM() else (params["strides"]["conv.4"], 2, 2)
    ...
    if getTO_CATEG():
        last_channels = len(getLABELS())
        activation_func = "softmax"
        shape_output = (getM(), getN(), last_channels)

    stride_5 = (1, 1, params["strides"]["conv.1"]) if getTIMELAST() or getUSE_PM()  else (params["strides"]["conv.1"], 1, 1)
    ...
    stride_6 = (1, 1, params["strides"]["conv.2"]) if getTIMELAST() or getUSE_PM() else (params["strides"]["conv.2"], 1, 1)
    ...
    stride_7 = (1, 1, params["strides"]["conv.3"]) if getTIMELAST() or getUSE_PM() else (params["strides"]["conv.3"], 1, 1)
    ...
    stride_8 = (1, 1, params["strides"]["conv.4"]) if getTIMELAST() or getUSE_PM() else (params["strides"]["conv.4"], 1, 1)
...
def VNet_Light(params):
    ...
    pool_size = (2,2,3) if getTIMELAST() else (3,2,2)
    ...
    pool_size = (2,2,5) if getTIMELAST() else (5,2,2)
    ...
    pool_size = (2,2,1) if getTIMELAST() else (1,2,2)
    ...
    l5_size = (2,2,1) if getTIMELAST() else (1,2,2)
    ...
    l6_size = (2,2,1) if getTIMELAST() else (1,2,2)
    ...
    l7_size = (2,2,5) if getTIMELAST() else (5,2,2)
    ...
    l8_size = (2,2,3) if getTIMELAST() else (3,2,2)
    ...
    k_size = (3,3,2) if getTIMELAST() else (2,3,3)
    stride = (1,1,2) if getTIMELAST() else (2,1,1)
    ...
    last_conv = tfa.layers.InstanceNormalization()(last_conv)
    stride = (1,1,3) if getTIMELAST() else (3,1,1)
    last_conv = layers.Conv3D(16,kernel_size=(3,3,3),activation="relu",padding='same',kernel_initializer=kernel_init,
                              strides=stride, kernel_constraint=kernel_constraint, bias_constraint=bias_constraint)(last_conv)
    last_conv = tfa.layers.InstanceNormalization()(last_conv)

    k_size = (3,3,5) if getTIMELAST() else (5,3,3)
    stride = (1,1,5) if getTIMELAST() else (5,1,1)
...
def VNet_Milletari_PMS(params, multiInput):
    ...
    list_input = []
    for pm in ["cbf", "cbv", "ttp", "mip", "mtt", "tmax"]:
        if pm in multiInput.keys() and multiInput[pm] == 1: list_input.append(layers.Input(shape=(getM(), getN(), 3), sparse=False))

    reshape_inputs = []
    for inp in list_input: reshape_inputs.append(layers.Reshape((getM(), getN(), 3, 1))(inp))
