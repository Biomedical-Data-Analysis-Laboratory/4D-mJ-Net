...
def splitDataset(nn, p_id, listOfPatientsToTrainVal, listOfPatientsToTest):
    ...
    if not nn.cross_validation:  # here only if: NO cross-validation set
        ...
        if nn.supervised:
            if nn.val["number_patients_for_testing"] > 0 or len(listOfPatientsToTest) > 0:
                ...
                if len(listOfPatientsToTest) > 0:  # if we already set the patient list in the setting file
                    ...
                    random.seed(631) # 631 - 2 // 43 for val_perc == 10% random.seed(43)  # use ALWAYS the same random indices
        ...
        if nn.val["number_patients_for_validation"] > 0:
            ...
            if constants.getVerbose():
                print("[INFO] - VALIDATION list LVO: {}".format([v for v in validation_list if "01_" in v or "00_" in v]))
                print("[INFO] - VALIDATION list Non-LVO: {}".format([v for v in validation_list if "02_" in v]))
                print("[INFO] - VALIDATION list WIS: {}".format([v for v in validation_list if "03_" in v]))
