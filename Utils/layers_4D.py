...
from tensorflow.keras.layers import Conv3D, Conv3DTranspose, Reshape, LeakyReLU
...
def Conv4D(inp, filters, kernel_size, strides=(1,1,1), padding='same', data_format='channels_last',
    ...
    Returns: The output tensor will be of shape: (b,k,z',t',x',y').
    """

    assert len(inp.get_shape().as_list()) == 6, "Tensor of shape (b,z,t,x,y,c) expected"
    assert len(kernel_size) == 4, "4D kernel size expected"
    assert padding in ['same','valid'], "Padding format other than same not implemented"
    ...
    else: (z_o, t_o, x_o, y_o) = (z_i, t_i, x_i, y_i)  # output size for 'same' convolution
        ...
        for j in range(reduce_i):
            ...
            if frame_conv3D is None:
                frame_conv3D = Conv3D(filters, kernel_size=(keep_k, x_k, y_k), padding=padding, strides=strides,
                                      activation='elu', use_bias=use_bias, kernel_initializer=kernel_initializer,
                                      bias_initializer=bias_initializer, kernel_regularizer=kernel_regularizer,
                                      bias_regularizer=bias_regularizer, activity_regularizer=activity_regularizer,
                                      kernel_constraint=kernel_constraint, bias_constraint=bias_constraint,
                                      trainable=trainable)

            out = frame_conv3D(input_layer)
            if frame_results[out_frame] is None: frame_results[out_frame] = out
            else: frame_results[out_frame] += out
...
def Conv4DTranspose(inp, filters, kernel_size, strides=(1,1,1), padding='same', data_format='channels_last',
    ...
    for i in range(reduce_k):
        ...
        for j in range(reduce_i):
            ...
            input_layer = Reshape(reshape_shape)(inp_reduced)
            if frame_conv3D is None:
                frame_conv3D = Conv3DTranspose(filters, kernel_size=(keep_k, x_k, y_k), padding=padding, strides=strides,
                                               activation='elu', use_bias=use_bias, kernel_initializer=kernel_initializer,
                                               bias_initializer=bias_initializer, kernel_regularizer=kernel_regularizer,
                                               bias_regularizer=bias_regularizer, activity_regularizer=activity_regularizer,
                                               kernel_constraint=kernel_constraint, bias_constraint=bias_constraint,
